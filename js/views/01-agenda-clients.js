...
function statusTone(status) {
  ...
  function renderClients() {
    ...
    const list = d.clients.filter(c => containsSearch(c.name,c.phone,c.cpf,c.city,c.profession)).sort((a,b)=>a.name.localeCompare(b.name,'pt-BR'));
    const cards = list.map(c => {
      ...
      if(mode==='list') return `<article class="list-row client-directory-row">
        ${profileAvatar({name:c.name},'small')}
        <button class="row-main text-button" data-action="view-client" data-id="${eattr(c.id)}"><strong>${esc(c.name)} ${c.archived?chip('Arquivada','warn'):''}</strong><span>${esc(c.phone || 'Sem telefone')} · ${esc(c.city || 'Cidade não informada')}</span><small>${atts.length} atendimento${atts.length!==1?'s':''} · ${pack} pacote${pack!==1?'s':''} · ${currency(spent)} pago</small></button>
        <div class="row-actions">${whatsapp?`<a class="icon-btn small whatsapp" href="${whatsapp}" target="_blank" rel="noopener" title="Abrir WhatsApp">${icon('phone',16)}</a>`:''}<button class="icon-btn small" data-action="edit-client" data-id="${eattr(c.id)}" title="Editar">${icon('edit',16)}</button><button class="icon-btn small" data-action="view-client" data-id="${eattr(c.id)}" title="Abrir ficha">${icon('chevron',16)}</button></div>
      </article>`;
      if(mode==='compact') return `<article class="client-compact-card">
        <button class="compact-card-main" data-action="view-client" data-id="${eattr(c.id)}">
          <span class="compact-card-top">${profileAvatar({name:c.name},'small')}<span><strong>${esc(c.name)}</strong><small>${esc(c.city || 'Cidade não informada')}</small></span></span>
        ...
        <footer>${whatsapp?`<a class="icon-btn small whatsapp" href="${whatsapp}" target="_blank" rel="noopener" title="WhatsApp">${icon('phone',16)}</a>`:''}<button class="icon-btn small" data-action="edit-client" data-id="${eattr(c.id)}" title="Editar">${icon('edit',16)}</button></footer>
      </article>`;
      return `<article class="client-card">
        <button class="client-card-main" data-action="view-client" data-id="${eattr(c.id)}">
          ...
          <span class="client-info"><strong>${esc(c.name)}</strong><small>${esc(c.phone || 'Sem telefone')}</small><small>${esc(c.city || 'Cidade não informada')}</small></span>
          ${icon('chevron',18)}
        </button>
    ...
    return `<section class="section-head">
      ...
      <div class="head-actions">${viewModeSwitcher('clients',mode)}<button class="btn primary" data-action="add-client">${icon('plus',18)} Nova cliente</button></div>
    </section>
    <section class="${containerClass}" data-view-content="clients">
      ${list.length ? cards : emptyState('Nenhuma cliente encontrada', SEARCH ? 'Tente pesquisar por outro nome ou telefone.' : 'Cadastre a primeira cliente.','add-client','Cadastrar cliente')}
    </section>`;
  }
