(() => {
  ...
  function projectRecord(item, state, bridge){
    ...
    const direction = item.type === 'expense' ? 'expense' : 'income';
    const amount = Math.round((Number(item.value) || 0) * 100) / 100;
    const entityId = String(item.id);
    const aggregateId = `${SPEC.sourceAppId}:${bridge.instanceId}:finance:${entityId}`;
    const payload = {
      aggregateId,
      entityType: 'finance',
      entityId,
      ...
      settled: status === 'paid',
      description: item.description || (direction === 'income' ? 'Receita da clínica' : 'Despesa da clínica'),
      category: item.category || (direction === 'income' ? 'Atendimento' : 'Outros'),
      paymentMethod: item.paymentMethod || '',
      clientName: item.clientName || '',
      origin: item.origin || 'Amanda Estética',
      notes: item.notes || '',
      externalReference: item.attendanceId || item.packageId || entityId,
  ...
  function projectRecords(state){
    ...
    const items = data && Array.isArray(data.finance) ? data.finance : [];
  ...
  function applyAcknowledgement(state, ack){
    ...
    const data = activeData(state);
    const sourceItems = data && Array.isArray(data.finance) ? data.finance : [];
  ...
  async function publish(state){
    ...
    try{
      ...
      try{
        ...
        if (handle && await ClinicStorage.ensurePermission(handle, false)){
      ...
      try{
        const drive = window.GoogleDriveClinic;
  ...
  window.AmandaBorionInterop = Object.freeze({
    spec: SPEC,
    start,
    schedule,
    publish,
